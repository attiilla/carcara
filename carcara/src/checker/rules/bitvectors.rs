...
pub fn concat(RuleArgs { conclusion, pool, .. }: RuleArgs) -> RuleResult {
    ...
    if res_vec.len() != size1 + size2 { return Err(CheckerError::Explanation(format!("Concat size {} different from sum of argument size {} + {}", res_vec.len(), size1, size2))); }
    ...
    let mut index = 0;
    for arg in res_vec {
        if index < size2 { assert_eq(&y[index], arg)?; }
        else { assert_eq(&x[index - size2], arg)?; }
